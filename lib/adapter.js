  ...
  var adapter = {
    ...
    query: function (connection, collection, query, data, cb) {
      ...
      adapter.connectConnection(connection, function __FIND__(err, uniqId) {
        ...
        var request = new mssql.Request(mssqlConnect);
        if (data) {  // because if not passed in the implementation is set to null
          for (var value in data){ // check each parameter: key:value
            if (data.hasOwnProperty(value)){ // is property
              if(query.indexOf(' IN ') > -1 || query.indexOf(' in ') > -1){
                ...
                ids_params = ids_params.slice(0, -1);
                query = query.replace('@0',ids_params);
                arr.forEach(function (element, index, array) {
                  request.input(index,arr[index]);
                });
              }else{
    ...
    // Raw Store Procedure Interface
    execute: function (connection, collection, query, data, cb) {
      if (_.isFunction(data)) {
        cb = data;
        data = null;
      }
      ...
      adapter.connectConnection(connection, function __FIND__(err, uniqId) {
        ...
        if (data) {  // because if not passed in the implementation is set to null
          //console.log(data);          
          for (var i in data){ // check each parameter          
        ...
        request.execute(query, function(err, recordset, returnValue, affected) {
          if (err) return cb(err);
          if (!connections[connection].persistent) {
            mssqlConnect && mssqlConnect.close();
          }            
